...
must_haves:
  truths:
    ...
    - "Edits to patterns loaded from browser persist to correct bank/slot"
  artifacts:
    ...
    - path: "Sources/Sequencer/Views/SequencerView.swift"
      provides: "Main sequencer container view with pattern location tracking"
      min_lines: 50
      contains: "editingPatternLocation"
  key_links:
    ...
    - from: "Sources/Sequencer/Views/SequencerView.swift"
      to: "PatternManager.savePattern"
      via: "editingPatternLocation tracks bank/slot for persistence"
      pattern: "editingPatternLocation.*savePattern"
---
...
Output: PianoRollGridView using SwiftUI Canvas for high-performance rendering, SequencerView as container with proper pattern location tracking for persistence.
</objective>
...
<task type="auto">
  <name>Task 3: Create SequencerView container with pattern location tracking</name>
  <files>
    Sources/Sequencer/Views/SequencerView.swift
  </files>
  <action>
Create SequencerView.swift as the main sequencer container with proper pattern location tracking for persistence:
...
/// Updated browser sheet that returns both pattern AND its location
...
IMPORTANT: The PatternBrowserSheet callback signature changed to include location.
Update PatternBrowserView.onSelect signature in Plan 06 to match:
`let onSelect: (Pattern, (bank: Int, slot: Int)) -> Void`
  </action>
  <verify>
Build succeeds: `cd /Users/trekkie/EurorackMIDI && swift build`
SequencerView displays transport, pattern info (with location indicator), track tabs, and grid
editingPatternLocation is set when loading from browser, used in onChange for persistence
  </verify>
  <done>
SequencerView contains TransportView, pattern info bar with location indicator, track selection tabs, piano roll grid, and bottom toolbar. Pattern location tracking via editingPatternLocation ensures edits persist to the correct bank/slot.
  </done>
</task>
...
5. SequencerView shows transport, pattern info (with slot identifier when loaded), track tabs, and grid
6. Track mute/solo buttons toggle state visually
7. Can change step count and swing
8. Pattern loaded from browser shows location (e.g., "A1") in info bar
9. Changes auto-save to correct bank/slot location
10. New pattern has no location until first save
</verification>
...
- editingPatternLocation tracks where pattern was loaded from
- onChange uses editingPatternLocation to save to correct bank/slot
- New patterns have nil location until explicitly saved
- Save button updates editingPatternLocation for new patterns
</success_criteria>
